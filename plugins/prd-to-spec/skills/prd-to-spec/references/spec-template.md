...
### 边界规则与交互后果(约束的交互推演;命中 ★3b 成员边界/取值约束时必填)
> 判据:源文档命中「仅 / 只能 / 必须同一 / 同源 / 不支持跨X / 互斥 / 至多N」类约束却无此表行 = **推演没冻结(识别≠冻结)**,不得进 spec-to-code。
| 约束 | 作用交互面 | 违反时行为(禁用入口 / 过滤不可选 / 切换即清空;用户已拍) | 极限态(空/唯一/超上限) | 出处 |
| --- | --- | --- | --- | --- |
| 〈如 同源:成员须同一数据账户〉 | 〈选择器/已选区/账户切换〉 | 〈三选一〉 | 〈…〉 | 〈出处〉 |
